...
import { Loan, Product, User } from "@/types";
      ...
      <CardContent className="py-2 flex-grow">
        <div className="grid gap-3">
          <div className="grid gap-1">
            ...
            <span className="text-sm font-medium">{user?.name || "Usuario"} {user?.studentId ? `(${user.studentId})` : ""}</span>
          </div>
          
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-sm text-muted-foreground block">Fecha de inicio:</span>
              <span className="text-sm font-medium">{format(new Date(loan.startDate), "dd/MM/yyyy")}</span>
            </div>
            <div>
          ...
          {loan.returnDate && (
            <div>
              ...
              <span className="text-sm font-medium">{format(new Date(loan.returnDate), "dd/MM/yyyy")}</span>
            </div>
          )}
        </div>
      </CardContent>
